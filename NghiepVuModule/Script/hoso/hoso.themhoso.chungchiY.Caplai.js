...
function fnLoadChungChiYCapLai() {
    var hdfHoSoId = $('input[name="hdfHoSoId"]').val();
    var request = JSON.stringify({ hosoId: hdfHoSoId });
    window.AjaxDungChung
        ("post",
            "/DesktopModules/MVC/NghiepVu/HoSo/GetChungChiHanhNgheY_CapLai_GetByHoSoID",
            window.moduleId,
            window.tabId,
...
function fnSaveChungChiYCapLai(linhVucId, thuThucId) {
    if (!validateform()) {
        return;
    }
    DisabledButtonSave();
