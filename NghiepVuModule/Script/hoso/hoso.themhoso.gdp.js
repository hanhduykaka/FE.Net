...
var _arrDelDSKho = [];
//Function xu ly
function fnSave_Duoc_GDP(linhVucId, thuTucId) {
    if (!validateform()) {
        return;
    }
    var formdata = PreparingSave_Duoc_GDP(linhVucId, thuTucId);
    window.AjaxDungChung
        ("post",
            "/DesktopModules/MVC/NghiepVu/NganhDuoc/SaveHoSoGDP",
            window.moduleId,
            window.tabId,
...
function PreparingSave_Duoc_GDP(linhVucId, thuTucId) {
    var parent = $(".div_tong");
    ...
    var THTGDPId = $('input[name="hdfId"]').val() == ""
        ? "0"
        : $('input[name="hdfId"]').val();
    objArr["THTGDPId"] = THTGDPId;
    ...
    formdata.append("dsKho", JSON.stringify(_arrDSKho));
    formdata.append("dsKhoDel", JSON.stringify(_arrDelDSKho));
    return formdata;
}
function fnLoad_Duoc_GDP() {
    if (Id)
        LoadGDPByID("GDP_XuLyLoadData");
    else if (HoSoID)
        LoadGDPByHoSoID("GDP_XuLyLoadData");
    else
        GDP_XuLyLoadData();
    ValidateDatePickers('input[name="NgayNhan"]', ['input[name="NgayHenTra"]', 'input[name="NgayCapChungNhan"]', 'input[name="NgayQuyetDinh"]']);
    $('input[name="SoChungChi"]').blur(GDP_onBlurSoChungChi);
    $('.btn_Xoa').off("click");
    $('.btn_Xoa').click(GDP_onClickBtnXoa);
    $('.icon_search_chung_chi').addClass("GDP");
    $('.icon_search_uu_diem').addClass("GDP");
    $('.icon_search_ton_tai').addClass("GDP");
    $('.icon_search_ket_luan').addClass("GDP");
    $('.input-select-chung-chi').addClass("GDP");
    $('.input-search-chung-chi').addClass("GDP");
    $('.input-select-ttkt').addClass("GDP");
    $('.input-search-ttkt').addClass("GDP");
    window.LoadDataInputText("hdfInDeXuat", "/DesktopModules/MVC/NghiepVu/NganhDuoc/InDeXuatGDP");
    window.LoadDataInputText("hdfInChungChi", "/DesktopModules/MVC/NghiepVu/NganhDuoc/InChungChiGDP");
}
function LoadGDPByHoSoID(funcXuly) {
    var request = JSON.stringify({ HoSoID: HoSoID });
    window.AjaxDungChung
      ("post",
          "/DesktopModules/MVC/NghiepVu/NganhDuoc/GetCN_GDPByHoSoID",
          window.moduleId,
          window.tabId,
          $("input[name='__RequestVerificationToken']").val(),
          request,
...
function LoadGDPByID(funcXuly) {
    var request = JSON.stringify({ THTGDPID: Id });
    window.AjaxDungChung
      ("post",
          "/DesktopModules/MVC/NghiepVu/NganhDuoc/GetCN_GDPByID",
          window.moduleId,
          window.tabId,
          $("input[name='__RequestVerificationToken']").val(),
          request,
...
function GDP_XuLyLoadData(result) {
    _arrDSKho = [];
    _arrDelDSKho = [];
    if (result) {
        $(".btn_In").parent().attr('hidden', false);
        $(".btn_Xoa").parent().attr('hidden', false);
        var objGDP=result.gDP;
        window.LoadDataInputText("hdfId", objGDP.THTGDPId);
        Id = objGDP.THTGDPId;
        //THÔNG TIN GIẤY CHỨNG NHẬN
        if (!HoSoID) {
            HoSoID = objGDP.HoSoID;
            window.LoadDataInputText("hdfHoSoId", objGDP.HoSoID);
            window.LoadHoSoByHoSoId(HoSoID);
        }
        window.LoadDataInputText("SoGiayChungNhan", objGDP.SoGiayChungNhan);
        window.setValueDateTimePicker("NgayCapChungNhan", objGDP.NgayCapChungNhan);
        var optionThoiHan = $('select[Name="ThoiHan"]').children('option');
        $.each(optionThoiHan, (index, ele) => {
            if ($(ele).text()== objGDP.ThoiHan)
                window.LoadDataSelect2("ThoiHan", $(ele).val());
        });
        window.LoadDataInputText("SoQuyetDinh", objGDP.SoQuyetDinh);
        window.setValueDateTimePicker("NgayQuyetDinh", objGDP.NgayQuyetDinh);
        //CƠ SỞ ĐƯỢC CẤP CHỨNG NHẬN
        window.LoadDataInputText("TenCoSo", objGDP.TenCoSo);
        window.LoadDataInputText("SoDKKD", objGDP.SoDKKD);
        window.LoadDataSelect2("TinhThanh01", objGDP.TinhThanhCSId);
        window.LoadComboHuyenByTinhId($("select[name='TinhThanh01']"), objGDP.TinhThanhCSId, objGDP.QuanCSId);
        window.LoadComboXaByHuyenId($("select[name='QuanHuyen01']"), objGDP.QuanCSId, objGDP.PhuongCSId);
        window.LoadDataInputText("SoNha01", objGDP.SoNhaCS);
        window.LoadDataInputText("DienThoai", objGDP.SoDienThoai);
        window.LoadDataInputText("Email", objGDP.Email);
        $.each(result.lstgDP_DSKho, (index, ele) => {
            _arrDSKho.push({
        ...
        LoadChungChiDuocBySoChungChi(objGDP.SoChungChi, "GDP_XuLyLoadOnlyDataChungChi");
        //PHẠM VI KINH DOANH
        window.LoadDataSelectPicker($("select[name='PhamViKinhDoanh']"), objGDP.PhamViKinhDoanhIds, objGDP.PhamViKinhDoanhs);
        //THÔNG TIN KIỂM TRA
        window.LoadDataInputText("TruongDoan", objGDP.TruongDoanKiemTra);
        window.setValueDateTimePicker("NgayKiemTra", objGDP.NgayKiemTra);
        $('textarea[name="UuDiem"]').val(objGDP.UuDiem);
        $('textarea[name="TonTai"]').val(objGDP.TonTai);
        $('textarea[name="KetLuan"]').val(objGDP.KetLuan);
...
function GDP_XuLyLoadOnlyDataHoSo() {
    if (!HoSoID || HoSoID == 0) {
        HoSoID = 0;
...
function GDP_XuLyLoadOnlyDataChungChi(result) {
    if (result) {
...
function GDP_onBlurSoChungChi(e) {
    var element = $(e.currentTarget);
    var txtVal = element.val();
    LoadChungChiDuocBySoChungChi(txtVal, "GDP_XuLyLoadOnlyDataChungChi");
}
...
function GDP_onClickBtnXoa(e) {

    var request = JSON.stringify({ THTGDPID: Id });
    window.AjaxDungChung
      ("post",
          "/DesktopModules/MVC/NghiepVu/NganhDuoc/DeleteHoSoGDPByTHTGDPID",
          window.moduleId,
          window.tabId,
          $("input[name='__RequestVerificationToken']").val(),
          request,
...
function GDP_CallSearchAdvanceFunc(isSearch) {
    var parent = $("#modal-ds-chung-chi");
    var divId = ".div_edit_table_parent";
    if (isSearch) {
        var objArr = window.GetAllInputOnDiv(parent);
        GDP_XuLyCall_showDataTable_Func(divId, objArr);
    } else {
        GDP_XuLyCall_showDataTable_Func(divId);
    }
}
function GDP_XuLyCall_showDataTable_Func(divId, objArr) {
    if (objArr)
        GDP_showDataTable(divId, objArr);
    else
        GDP_showDataTable(divId);
};
function GDP_showDataTable(divId, chungchi) {
           ...
           columns: [
               {
                   ...
                   "orderable": false,
                   "width": "5%",
                   "className": "text-center  cursor-pointer GDP_chk_chon_chung_chi "
               },
               {
                   "render": function (data, type, full, meta) {
                       return full.RowNo;
                   ...
                   "orderable": false,
                   "className": "word-break text-center cursor-pointer GDP_xem_chi_tiet"
               },
               { "data": "SoChungChi", "width": null, "orderable": false, "className": "word-break cursor-pointer GDP_xem_chi_tiet" },
               {
                   "render": function (data, type, full, meta) {
                       if (full.NgayCap)
               ...
               { "data": "HoTen", "width": "10%", "orderable": false, "className": "word-break cursor-pointer GDP_xem_chi_tiet" },
               { "data": "SoGiayTo", "width": null, "orderable": false, "className": "word-break cursor-pointer GDP_xem_chi_tiet" },
               { "data": "ChoO_DiaChi", "width": "20%", "orderable": false, "className": "word-break cursor-pointer xGDP_em_chi_tiet" },
               { "data": "PhamViHoatDongChuyenMons", "width": "20%", "orderable": false, "className": "word-break cursor-pointer GDP_xem_chi_tiet" },
               //{ "data": "TrangThaiGiayPhep", "width": "20%", "orderable": false, "className": "word-break cursor-pointer GDP_xem_chi_tiet" }
           ],
...
function GDP_XuLyChonSoChungChi(soChungChi) {
    $('#modal-ds-chung-chi').modal('hide');
    LoadChungChiDuocBySoChungChi(soChungChi, "GDP_XuLyLoadOnlyDataChungChi");
}
$(document).on
("click",
    ".GDP_xem_chi_tiet",
...
$(document).on
("click",
    ".input-search-chung-chi.GDP",
    (e) => {
        $(".input-select-chung-chi").attr("data-sochungchi", "");
        GDP_CallSearchAdvanceFunc(true);
    });
$(document).on
("dblclick",
    ".GDP_xem_chi_tiet",
    (e) => {
        ...
        var parentTr = element.closest("tr");
        var soChungChi = parentTr.attr("data-sochungchi");
        GDP_XuLyChonSoChungChi(soChungChi);
    });
$(document).on
("click",
    ".input-select-chung-chi.GDP",
    (e) => {
...
$(document).on
("click", ".icon_search_chung_chi.GDP", (e) => {
    $('#modal-ds-chung-chi').modal('toggle');
    $(".input-select-chung-chi").attr("data-sochungchi", "");
    GDP_CallSearchAdvanceFunc(false);
});
//Fuction danh sach kho
function GDP_LoadDSKho() {
    var table = $("#tbl_danh_sach_kho_bao_quan");
    if (_arrDSKho.length > 0) {
        var html = "";
        $.each
        (_arrDSKho,
            function (i, item) {
                item.rowid = window.newGUIDFunc();
                html += html_Load_tbl_danh_sach_kho_bao_quan(item,i+1);
            });
...
function html_Load_tbl_danh_sach_kho_bao_quan(objData,index) {
    var html = "";
    html += '<tr data-id="' + objData.rowid + '" data-khoid="' + objData.THTGDPDSKhoId + '" class="font-size-13">';
    html += '    <td name="stt_' + objData.rowid + '">' + index + '</td>';
    html += '<td class="input-get line-height-1_1-Em" data-name="TenKho" data-value=' + objData.TenKho + '> ' + objData.TenKho + ' </td>';
    html += '<td class="input-get line-height-1_1-Em" data-name="DiaChiKho" data-value=' + objData.DiaChiKho + '> ' + objData.DiaChiKho + ' </td>';
    html += '<td class="fa fa-edit cursor-pointer color-blue font-size-20 table-cell syt-edit-btn line-height-1_1-Em"';
    html += ' data-khoid="' + objData.THTGDPDSKhoId + '" data-tenkho="' + objData.TenKho + '" data-diachikho="' + objData.DiaChiKho + '"';
    html += ' data-id="' + objData.rowid + '" title="Sửa"></td>';
...
function html_gen_edit_danh_sach_kho_bao_quan(data) {
    var html = "";
    html += '   <tr class="font-size-13" data-khoid="' + (data ? data.khoid : '0') + '" data-id="' + (data ? data.id : window.newGUIDFunc()) + '" name="tr-edit">';
    html += '<td> </td>';
    html += '   <td>';
    html += '<input class="width-100 syt-radius-3 min-h-30 paddingMin txt_ten_kho"' +
        ' type="text" ' + (data ? "value=\"" + data.tenkho + "\"" : '') + '/>';
    html += '    </td>';
    html += '   <td>';
    html += '<input class="width-100 syt-radius-3 min-h-30 paddingMin txt_dia_chi_kho"' +
      ' type="text" ' + (data ? "value=\"" + data.diachikho + "\"" : '') + '/>';
    html += '    </td>';
    html += '   <td>';
...
function save_edit_danh_sach_kho_bao_quan(element) {
    var parentTr = $(element).closest("tr");
    var rowid = parentTr.attr('data-id');
    var obj = _arrDSKho.find(x=>x.rowid == rowid);
    if (!obj) {
        var newobj = {
            THTGDPDSKhoId: parentTr.attr('data-khoid')
            , TenKho: (parentTr.find('.txt_ten_kho')).val()
            , DiaChiKho: (parentTr.find('.txt_dia_chi_kho')).val()
            , rowid: parentTr.attr('data-id')
        };
        _arrDSKho.push(newobj);
    }
    else {
        obj.THTGDPDSKhoId = parentTr.attr('data-khoid');
        obj.TenKho = (parentTr.find('.txt_ten_kho')).val();
        obj.DiaChiKho = (parentTr.find('.txt_dia_chi_kho')).val();
        obj.THTGDPDSKhoId = parentTr.attr('data-id');
        obj.rowid = parentTr.attr('data-id');
    }
    GDP_LoadDSKho(_arrDSKho);
}
function del_tr_danh_sach_kho_bao_quan(element) {
    var parentTr = $(element).closest("tr");
    var rowid = parentTr.attr('data-id');
    var khoid = parentTr.attr('data-khoid');
    if (khoid)
        _arrDelDSKho.push({ THTGDPDSKhoId: khoid });
    _arrDSKho = _arrDSKho.filter(x=>x.rowid != rowid);
    GDP_LoadDSKho(_arrDSKho);
}
////Fuction modal goi y
function GDP_LoadDataGoiY(loaiGoiYID, funcXuly) {
...
function GDP_LoadTableGoiY(data, loaiGoiYID) {
...
function html_Load_tbl_GoiY(loaiGoiYID, objData) {
    ...
    html += '<td class="input-get line-height-1_1-Em" data-name="Ten" data-value=' + objData.Ten + '> ' + objData.Ten + ' </td>';
    html += '<td class="fa fa-edit cursor-pointer color-blue font-size-20 table-cell syt-edit-btn line-height-1_1-Em"';
    ...
    window.AjaxDungChung
     ...
     (function (data) {
         if (data && (data >= 0 || data.Content >= 0)) {
             ...
             GDP_LoadDataGoiY($('input[name="hdfLoaiGoiYID"]').val(), 'GDP_LoadTableGoiY');
         }
         else {
...
$(document).on
("click", ".icon_search_uu_diem.GDP", (e) => {
    $('#modal-ds-goi-y').modal('toggle');
    window.LoadDataInputText("goi_y_seacrh", '');
    window.LoadDataInputText("hdfLoaiGoiYID", 2);
    GDP_LoadDataGoiY(2, 'GDP_LoadTableGoiY');
});
$(document).on
("click", ".icon_search_ton_tai.GDP", (e) => {
    $('#modal-ds-goi-y').modal('toggle');
    window.LoadDataInputText("goi_y_seacrh", '');
    window.LoadDataInputText("hdfLoaiGoiYID", 6);
    GDP_LoadDataGoiY(6, 'GDP_LoadTableGoiY');
});
$(document).on
("click", ".icon_search_ket_luan.GDP", (e) => {
    $('#modal-ds-goi-y').modal('toggle');
    window.LoadDataInputText("goi_y_seacrh", '');
    window.LoadDataInputText("hdfLoaiGoiYID", 5);
    GDP_LoadDataGoiY(5, 'GDP_LoadTableGoiY');
});
$(document).on
("click",
    ".input-search-ttkt.GDP",
    (e) => {
        GDP_LoadDataGoiY($('input[name="hdfLoaiGoiYID"]').val(), 'GDP_LoadTableGoiY');
    });
$(document).on
