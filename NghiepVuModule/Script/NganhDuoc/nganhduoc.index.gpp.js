       ...
       window.AjaxDungChung
            ...
            (function(data) {
                if (data) {
                    ...
                    var url = data.TabID + '/moduleId/' + data.ModuleID + '/linhVucId/' + 2 + '/thuTucId/' + 18
...
function showDataTable(divId, chungchi) {
    ...
    var tblDataTable = $(tableid).DataTable
       ({
           ...
           "ajax": {
               "type": "Post",
               "url": "/DesktopModules/MVC/NghiepVu/NganhDuoc/TraCuuGPPByPage",
               "dataType": "json",
               data: chungchi,
               headers: {
                   "ModuleId": moduleId,
           ...
           'createdRow': function (row, data) {
               $(row).attr('data-id', data.THTGPPId);
           },
           'lengthChange': true,
           "ordering": false,
           'searching': false,
...
$(document).on('click', "tbody .xem_chi_tiet", (e) => {
    ...
    var THTGPPId = parentTr.attr("data-id");
    window.AjaxDungChung
        ("post",
            "/DesktopModules/MVC/NghiepVu/HoSo/GetModule",
            window.moduleId,
        ...
        (function (data) {
            if (data) {
                ...
                var url = data.TabID + '/moduleId/' + data.ModuleID  + '/linhVucId/'
                    + 2+ '/thuTucId/' + 18 + "/id/" + THTGPPId +'/prev/' + uri
                    + '/controller/HoSo/action/ThemHoSo';
                location.href = url;
            }
        });
...
((e) => {
    ...
    window.AjaxDungChung
       ("post",
         "/DesktopModules/MVC/NghiepVu/NganhDuoc/NganhDuoc_CN_GPP_XuatDanhSach",
           window.moduleId,
           window.tabId,
           $("input[name='__RequestVerificationToken']").val(),
           request,
...
((e) => {
    ...
    window.AjaxDungChung
       ("post",
         "/DesktopModules/MVC/NghiepVu/NganhDuoc/NganhDuoc_CN_GPP_CongBoWebsite",
           window.moduleId,
           window.tabId,
           $("input[name='__RequestVerificationToken']").val(),
           request,
